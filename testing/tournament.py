...
class Bracket:
    def __init__(self, filename, year):
        ...
        path = "../data/data_matrices/tournament_matrices/"
        file = path + filename
        self.path = path
        self.file = file
        self.year = year
        team_names = pd.read_csv('../data/data/kaggle_raw/Teams.csv')
        team_seeds = pd.read_csv('../data/data/kaggle_raw/NCAATourneySeeds.csv')
        df = pd.read_csv(file)
...
def predict_tournament(prediction_function, path, filename, year = 2017):
